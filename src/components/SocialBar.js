    ...
    render={data => (
      <div className={social.wrap}>
        {icons.map(i => {
          ...
          return (
            <a key={`icon_${i.name}`} aria-label={'go to '+i.name} rel="noopener noreferrer" target="_blank" href={checkIcon[i.name + '_link']} className={social.icon} style={{ height: 20, width: 20 }}>
